...
- Existing ReidCLI architecture:
  - `src/reidcli/tasks/`: per-session task persistence and task status state machine.
  - `src/reidcli/runtime/orchestrator.py`: session/task/agent coordination.
  - `src/reidcli/ui/commands.py`: slash command routing and help/completion source.
  - `src/reidcli/ui/render.py`: Rich rendering helpers.
  - `src/reidcli/ui/app.py`: full-screen TUI, slash command dispatch, completion menu.
  - `src/reidcli/session/store.py`: per-session persistence layout.

## Product Intent

Add a first-class `/goal` surface for long-horizon work in ReidCLI.
...
Create `src/reidcli/goals/models.py`.
...
Create `src/reidcli/goals/store.py`, mirroring `TaskStore`.

Path:

```text
~/.reidcli/sessions/<session-id>/goals.json
```
...
Update `src/reidcli/runtime/orchestrator.py`.
...
Add `/goal` to `SLASH_COMMANDS` in `src/reidcli/ui/commands.py`:
...
Update `src/reidcli/ui/render.py`.
